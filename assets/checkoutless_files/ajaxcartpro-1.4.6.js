...
function addSubmitEvent(mine)
{
    try
    {
        ...
        else if (typeof mine != 'undefined' && !AW_ACP.disabled && !awacpclass.isCartConfigurePage())
        {
            mine.submit = function(url){
                if(this.validator && this.validator.validate()){
                    // add realytics
            ...
            mine.form.onsubmit = function() {
                mine.submit();
                return false;
            };


            productAddToCartForm2.submit = function(url){
                if(this.validator && this.validator.validate()){
                    ajaxcartsend('awacp=1', 'form', this, '');
                }
                return false;
            ...
            productAddToCartForm2.form.onsubmit = function() {
                //productAddToCartForm2.submit();
                return false;
            };
        }
    }
    ...
    if (type == 'form') {
        ...
        aForm.request({
            onCreate: function(){
                ...
                jQuery('.popup-holder3 .popup').show();
                jQuery('#cart-header-container').closest('.dropdown').addClass('open');
            ...
            onComplete:  function(resp) {
                ...
                if (resp.r != 'success') {
                    ...
                    updateWishlist(resp);
                    updateWishlistTopLinks(resp);
                    jQuery('.monMiniCart').trigger('minicart:update');
                }
            }
        })
